...
**手機**(自動切換成觸控介面,建議橫放):左下十字鍵 ◀ ▶ 移動、▲ 跳、▼ 往下跳;右下搖桿推哪裡就朝哪裡自動射箭,🐰 兔劍、衝 月光衝刺。
...
遊戲的數值和判定邏輯(重力、單向平台、兔劍範圍、冷卻、光照規則、勝負條件……)都放在 `src/logic.js`,並用 Node 內建的測試工具寫了 30 個單元測試,每個測試都對應 `SPEC.md` 的一條規則:
